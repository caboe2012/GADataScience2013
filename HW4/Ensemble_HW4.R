...
## Load the necessary libraries in order to plot the Survival Analysis charts and Decision Tree ##
library(survival)
library(KMsurv)
library(party)
...
head(data)
# Create two empty vectors that will be used to store the overall analysis resutls for both Total Errors
...
logit_dtree.nfold <- function(seed, n.folds) {
...
# since the time of the most recent heart attack 
		...
		# Decision Tree Model
		whas_tree <- ctree(fstat ~ age + bmi + chf + miord + lenfol, data=train.data)
		
		cat('\n', 'Summary of fold ', fold,' Training Analysis Below', '\n', sep = "")

#######################################
### Display Training Survival Chart ###
#######################################	
...
## Run the Logistic Model against the new test data and add column to the new.data frame
		new.data$logit.mort.prob <- predict(logit.fit, newdata=new.data, type='response')
## Run the Decision Tree Model against the new test data and add column to the new.data frame		
		new.data$dtree.mort.prob <- predict(whas_tree, new.data, type='response')
## Ensemble the results of the two analyses by taking the linear average of their results
		new.data$mort.prob <- ((new.data$logit.mort.prob + new.data$dtree.mort.prob) / 2)
...
# Juxtapose the test data set's known survival statistics column with the model's predicted survival stats.
...
#########################################
###  Analyze the Generalization Error ###
#########################################	
...
###############################################
### Display Total Generalization Error Info ###
###############################################	
