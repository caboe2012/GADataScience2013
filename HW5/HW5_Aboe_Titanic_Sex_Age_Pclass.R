...
# Train the Model ##
		logit.fit <- glm(Survived ~ Sex + Age + Pclass, family='binomial', data=train.data)
...
## Determine the Generaliztion error for my model by applying the Test Fold ##
		new.data <- with(test.data, data.frame(Sex=Sex, Age=Age, Pclass=Pclass))
		...
		kaggle.data <- with(kaggle, data.frame(Sex=Sex, Age=Age, Pclass=Pclass))
		...
		submit$Survived <- round(result)
		print (submit)
		if (fold == 5) {
			write.csv(submit, file ="C:\\Caboe_Sex_Age_PClass_Fold5.csv", row.names=FALSE)	
		}
		if (fold == 2){
			write.csv(submit, file ="C:\\Caboe_Sex_Age_PClass_Fold2.csv", row.names=FALSE)	
		}
		write.csv(submit, file ="C:\\Caboe_Sex_Age_PClass_Fold9.csv", row.names=FALSE)	
